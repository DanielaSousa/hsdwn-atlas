...
bool
OFSwitch13Port::PortDeviceIsWiFi(void)
{
...
OFSwitch13Port::NotifyConstructionCompleted()
{
    ...
    Ptr<WifiNetDevice> wifiDev = m_netDev->GetObject<WifiNetDevice>();
    NS_ABORT_MSG_IF(!csmaDev && !virtDev && !wifiDev,
                    "NetDevice must be CsmaNetDevice or VirtualNetDevice or WifiNetDevice.");
    ...
    {
        wifiDev->SetOpenFlowReceiveCallback(MakeCallback(&OFSwitch13Port::Receive, this));
    }
    else
    {
        NS_ASSERT(virtDev);
...
// bool OFSwitch13Port::Legacy2ofswitch (Ptr<NetDevice> device, Ptr<const Packet> packet,
...
//   //check ip protocol next

//   uint16_t next_protocol;
//   uint16_t olsr = 0;
//   if(next_protocol== olsr){
//     //send to olsr routing
//   }
//   else{
//     Receive(device, packet, protocol, from, to, packetType);
//   }
...
{
    // packet ns3:packet
    // protocol : Next protocol header value. the 16 bit protocol number associated with this
    // packet. packetType (broadcast/multicast/unicast/otherhost) NOTA:  Mac48Address
    ...
    packet->Print(stream);
    // stream << "\n";
    // if (packet->GetUid() == 1508)
    // {
    //     std::cout << "receive Packet in device" << device->GetAddress() << std::endl;
    // }

    NS_LOG_DEBUG("[OFSwitch13Port::Receive] " << packet->GetSize() << " size header" << protocol
                                              << "\n Packet: [  " << stream.str() << "  ]\n");
    ...
    // m_openflowDev->ReceiveFromSwitchPort(localPacket, m_portNo, tunnelId); // official version
    m_openflowDev->ReceiveFromSwitchPort(localPacket, m_portNo, protocol, from, to, tunnelId);

    return true;
}

bool
OFSwitch13Port::SendNormalAction(Ptr<const Packet> packet, uint32_t queueNo, uint64_t tunnelId)
{
    ...
    if (PortDeviceIsWiFi())
    {
        ...
        Ptr<WifiNetDevice> wifiDev = m_netDev->GetObject<WifiNetDevice>();
        status =
            wifiDev->Forward_OFPP_NORMAL(packetCopy, header.GetSource(), header.GetDestination());
    ...
    else
    {
        ...
        Ptr<CsmaNetDevice> csmaDev = m_netDev->GetObject<CsmaNetDevice>();
        status = csmaDev->Forward_OFPP_NORMAL(packet, header.GetSource(), header.GetDestination());
    }
...
OFSwitch13Port::Send(Ptr<const Packet> packet, uint32_t queueNo, uint64_t tunnelId)
{
    ...
    std::ostringstream stream;
    packet->Print(stream);
    NS_LOG_INFO("OFSwitch13Port::Send " << stream.str());
