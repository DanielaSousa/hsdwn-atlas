...
BufferFromPacket(Ptr<const Packet> packet, size_t bodyRoom, size_t headRoom)
{
    ...
    // std::cout << "BufferFromPacket (packetSize == bufferSize):" << pktSize << std::endl;
...
Ptr<Packet>
myPacketFromBuffer(struct ofpbuf* buffer)
{
    NS_LOG_FUNCTION_NOARGS();

    // eu sei que vem ethernet, depois llc, e depois algo
