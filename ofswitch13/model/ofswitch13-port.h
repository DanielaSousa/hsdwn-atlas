...
class OFSwitch13Port : public Object
{
  ...
  /**
   * Check if PortDevice is type WifiNetDevice
   * \return True if it is, False if not
  */
  bool PortDeviceIsWiFi();
  ...
  bool Send (Ptr<const Packet> packet, uint32_t queueNo = 0,
             uint64_t tunnelId = 0);
  bool SendNormalAction (Ptr<const Packet> packet, uint32_t queueNo = 0,
                      uint64_t tunnelId = 0);
...
private:
  ...
  /**
   * @brief Called when a packet is received on this OpenFlow switch port by the
   ...
   * @return true
   */
  // bool Legacy2ofswitch (Ptr<NetDevice> device, Ptr<const Packet> packet,
  //               uint16_t protocol, const Address &from, const Address &to,
  //               NetDevice::PacketType packetType);
