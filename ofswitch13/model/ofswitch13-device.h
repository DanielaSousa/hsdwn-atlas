...
class OFSwitch13Device : public Object
{
  ...
  public:
    ...
    // /**
    //  * @brief add routing helper to device
    //  *
    //  * @param list
    //  */
    //   void AddList(Ipv4ListRoutingHelper &list);
    ...
    /**
     * @brief send flow statistics to the controller
     *
     * @param length
     * @param data
     */
    void SendStatstoCtrl(int length, uint8_t* data, uint32_t exp_type);

    /**
     * @brief Get the Ip Interfaces object
     *
     * @return std::vector<Ipv4Address>
     */
    std::vector<Ipv4Address> GetIpInterfaces();
  ...
  private:
    ...
    /**
     * @brief Check if new entries on olsr/device exists and send port status with hw_addr + ip
     * (optional). This method reschedules itself at every second.
     */
    void GraphPortsUpdate(struct datapath* dp);
    ...
    bool SendToReservedPort(struct packet* pkt, uint32_t queueNo = 0);
    ...
    bool sentExperimenter = false;

    // The workFlow Monitor Callback
    std::vector<Callback<void, int>> m_workflowCallback;
