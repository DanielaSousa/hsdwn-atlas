...
Ptr<OFSwitch13Device>
OFSwitch13Helper::InstallSwitch (Ptr<Node> swNode, NetDeviceContainer &swPorts, Ipv4ListRoutingHelper &list)
{
  NS_LOG_FUNCTION (this << swNode);

  // Install the OpenFlow device into switch node.
  std::cout << "bla" << std::endl;
  Ptr<OFSwitch13Device> openFlowDev = InstallSwitch (swNode, list);
  ...
  // openFlowDev->dev_olsr = DynamicCast<olsr::RoutingProtocol> (temp);
  // NS_ASSERT_MSG (openFlowDev->dev_olsr, "No olsr::RoutingProtocol");
  // Add switch ports.
  NetDeviceContainer::Iterator it;
  for (it = swPorts.Begin (); it != swPorts.End (); it++)
    {
...
Ptr<OFSwitch13Device>
OFSwitch13Helper::InstallSwitch (Ptr<Node> swNode, Ipv4ListRoutingHelper &list)
{
  NS_LOG_FUNCTION (this << swNode);
