...
struct RoutingTableEntry
{
    ...
    bool operator==(const RoutingTableEntry& r) const
    {
        if (destAddr == r.destAddr && nextAddr == r.nextAddr && interface == r.interface &&
            distance == r.distance)
        {
            return true;
        }
...
class RoutingProtocol : public Ipv4RoutingProtocol
{
  public:
    ...
    /**
     * @brief Get the Aux Table object from olsr
     *
     * @return std::map< Ipv4Address, std::set<Ipv4Address> >
     */
    std::map<Ipv4Address, std::set<Ipv4Address>> GetAuxTable();

    std::set<Ipv4Address> GetNeighbours();
  ...
  private:
    ...
    /// @brief Routing table recalculation
    TracedCallback<uint32_t> m_routingTableRecalculated;

    /// Provides uniform random variables.
    Ptr<UniformRandomVariable> m_uniformRandomVariable;

    std::map<Ipv4Address, std::set<Ipv4Address>> aux_table;
};
