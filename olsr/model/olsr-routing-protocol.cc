...
RoutingProtocol::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::olsr::RoutingProtocol")
            ...
            .AddTraceSource("RoutingTableRecalculated",
                            "The OLSR routing table has been recalculated.",
                            MakeTraceSourceAccessor(&RoutingProtocol::m_routingTableRecalculated),
                            "ns3::olsr::RoutingProtocol::TableRecalculatedTracedCallback");
...
std::map<Ipv4Address, std::set<Ipv4Address>>
RoutingProtocol::GetAuxTable()
{
    // std::cout << "Direct neighbours Table "
    //           << "Node: " << m_ipv4->GetObject<Node>()->GetId() << ": ";
    // for (auto it = m_table.begin(); it != m_table.end(); it++)
    // { // std::map<Ipv4Address, RoutingTableEntry>
    //     if (it->second.destAddr == it->second.nextAddr)
    //     {
    //         std::cout << it->second.destAddr << " ";
    //     }
    // }
    // std::cout << std::endl;
...
RoutingProtocol::RecvOlsr(Ptr<Socket> socket)
{
    ...
    // std::cout << "Aux Entry [";
    // for (auto it = aux_table[m_mainAddress].begin(); it != aux_table[m_mainAddress].end(); it++)
    // {
    //     std::cout << *it;
    // }
    // std::cout << std::endl;
...
RoutingProtocol::RoutingTableComputation()
{
    ...
    // copy table to compare if changes occour
    std::map<Ipv4Address, RoutingTableEntry> copy_table(m_table);
    ...
    if (copy_table != m_table)
    {
        m_routingTableChanged(GetSize());
    }
    m_routingTableRecalculated(GetSize());
}
...
RoutingProtocol::NeighborLoss(const LinkTuple& tuple)
{
    ...
    // remove aux_table
    aux_table[m_mainAddress].erase(tuple.neighborIfaceAddr);
...
RoutingProtocol::LinkTupleTimerExpire(Ipv4Address neighborIfaceAddr)
{
    ...
    if (tuple->time < now)
    {
        ...
        // std::cout << "RemoveLinkTuple" << tuple->localIfaceAddr << " -> "
        //           << tuple->neighborIfaceAddr << std::endl;
    }
...
RoutingProtocol::TopologyTupleTimerExpire(Ipv4Address destAddr, Ipv4Address lastAddr)
{
    ...
    if (tuple->expirationTime < Simulator::Now())
    {
        ...
        // std::cout << "RemoveTopologyTuple " << tuple->destAddr << " -> " << tuple->lastAddr
        //           << std::endl;
    }
    else
    {
...
RoutingProtocol::RemoveEntry(const Ipv4Address& dest)
{
    ...
    // std::cout << "RoutingProtocol::RemoveEntry " << dest << std::endl;

    // // remove entry from auxiliary table
    // for (std::map<Ipv4Address, std::set<Ipv4Address>>::iterator it = aux_table.begin();
    //      it != aux_table.end();
    //      ++it)
    // {
...
{
    // std::cout << "RoutingProtocol::FindSendEntry [" << entry.destAddr << "," << entry.nextAddr
    //           << "]" << std::endl;
    ...
    while (outEntry.destAddr != outEntry.nextAddr)
    {
        ...
        // std::cout << "[" << outEntry.destAddr << "," << outEntry.nextAddr << "]" << std::endl;
    }
    return true;
}
                            ...
                            const ErrorCallback& ecb)
{
    ...
    // for (auto it = m_table.begin(); it != m_table.end(); it++)
    // {
    //     std::cout << "Entry " << it->first << ": [" << it->second.destAddr << ","
    //               << it->second.nextAddr << "]" << std::endl;
    // }

    // std::cout << "Aux Entry [";
    // for (auto it = aux_table[Ipv4Address("10.1.1.7")].begin();
    //      it != aux_table[Ipv4Address("10.1.1.7")].end();
    //      it++)
    // {
    //     std::cout << *it;
    // }
    ...
    // ----
    if (Lookup(header.GetDestination(), entry1)) // aqui busca o m_table[dst]
    {
        // std::cout << "Entry1 : [" << entry1.destAddr << "," << entry1.nextAddr << "]" <<
        // std::endl;
    ...
    for (uint32_t i = 0; i < m_ipv4->GetNInterfaces(); i++)
    {
        for (uint32_t j = 0; j < m_ipv4->GetNAddresses(i); j++)
        {
            if (m_ipv4->GetAddress(i, j).GetLocal() == interfaceAddress)
            {
                ...
                // std::cout << "RoutingProtocol::AddEntry" << dest << next << interfaceAddress
                //           << distance;

                // ---- populate auxiliary table for topology discovery in sdn
                if (distance < 2) // direct neighbour
                {
                    aux_table[interfaceAddress].insert(next);
                    // std::cout << " INSERT" << std::endl;
                }
                else // not direct neighbour
                {
                    // if dest is already in aux_table --> delete
                    if (aux_table[interfaceAddress].find(dest) != aux_table[interfaceAddress].end())
                    {
                        aux_table[interfaceAddress].erase(dest);
