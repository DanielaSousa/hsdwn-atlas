...
main(int argc, char* argv[])
{
    ...
    {
        std::cout << "\n legacy " << c.Get(i)->GetNDevices() << " "
                  << c.Get(i)->GetDevice(0)->GetAddress() << std::endl;
        ipv4 = c.Get(i)->GetObject<Ipv4>();
        for (size_t j = 0; j < ipv4->GetNInterfaces(); j++)
        {
            std::cout << ipv4->GetAddress(j, 0) << std::endl;
        }
    }
