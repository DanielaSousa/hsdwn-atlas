...
{
    NS_LOG_FUNCTION(this << m_wifiPhy << wifiPhy);
    m_wifiPhy = wifiPhy;
...
PhyEntity::EndPreambleDetectionPeriod(Ptr<Event> event)
{
    ...
    // std::cout << "SNR(dB)=" << RatioToDb(snr) << " at end of preamble detection period"
    //           << std::endl;
    // std::cout << (!m_wifiPhy->m_preambleDetectionModel && maxRxPowerW > 0.0) << "|"
