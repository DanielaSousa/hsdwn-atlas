...
YansWifiChannel::Send(Ptr<YansWifiPhy> sender, Ptr<const WifiPpdu> ppdu, double txPowerDbm) const
{
    ...
    for (PhyList::const_iterator i = m_phyList.begin(); i != m_phyList.end(); i++)
    {
        if (sender != (*i))
        {
            ...
            NS_LOG_ERROR("propagation: txPower="
...
YansWifiChannel::Receive(Ptr<YansWifiPhy> phy, Ptr<const WifiPpdu> ppdu, double rxPowerDbm)
{
    ...
    // std::cout << "YansWifiChannel::Receive" << rxPowerDbm + phy->GetRxGain() << " " << rxPowerDbm
    //           << " " << phy->GetRxGain() << std::endl;
    // if (rxPowerDbm + phy->GetRxGain() > -80 && Simulator::Now() > Seconds(26.055))
    // {
    //     std::cout << "DEu merda" << rxPowerDbm << " " << phy->GetRxGain() << std::endl;
    // }
