...
/**
  ...
  bool Forward_OFPP_NORMAL(Ptr<const Packet> origin_packet, Mac48Address from, Mac48Address to);

  bool openflow_enabled = false;
  void SendToPipelineLocal(Ptr<const Packet> packet);

  /**
   * @brief Workflow monitor function
   *
   * @param signal 0 for sent message, 1 for received message from controller
   */
  void WorkflowMonitor(int signal);
