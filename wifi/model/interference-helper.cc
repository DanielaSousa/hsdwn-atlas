                                                ...
                                                const FrequencyRange& range) const
{
    ...
    // std::cout << noiseInterferenceW << std::endl;
    NS_ABORT_IF(m_niChanges.count(range) == 0);
    auto niIt = m_niChanges.at(range).find(band);
    NS_ABORT_IF(niIt == m_niChanges.at(range).end());
    ...
    for (; it != niIt->second.end() && it->first < Simulator::Now(); ++it)
    {
        ...
        // {
        //     std::cout << "InterferenceHelper::CalculateNoiseInterferenceW (" << Simulator::Now()
        //               << ")" << it->second.GetPower() << " " << event->GetRxPowerW(band)
        //               << std::endl;
        // }
    }
                                 ...
                                 const FrequencyRange& range) const
{
    ...
    // std::cout << event->GetRxPowerW(band) << "W, " << noiseInterferenceW << "noise , "
    //           << channelWidth << " width, " << nss << " " << std::endl;
    return snr;
}
